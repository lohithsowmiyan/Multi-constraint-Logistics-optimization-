...
class MMT:
    ...
    def txt_solution(self, route, order):
        ...
        txt = { "Number of goods" : str(order['Order Number'].count()),
        ...
        for i in range(order.shape[0]):
            ...
            for j in solution:
                route_txt = { "No":str(a),
                              ...
                              "From" : j[0],
                              "To" : j[1],
                              "By" : travelMode[(j[0], j[1])]}
                a=a+1
                total_route.append(route_txt)
            trip["Route"] =  total_route
            trip_tot.append(trip)
        txt["Goods"] = trip_tot
...
def find_shortest_path(input):
    route = transform("model data-4.xlsx")
    print(input)
    order = pd.DataFrame(input)
    print(order)
    print(order.info())
    m = MMT()
    ...
    return txt
    #with open("routes.json", "w") as text_file:
    #    json.dump(txt,text_file)
